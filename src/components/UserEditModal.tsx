...
const UserEditModal: React.FC<UserEditModalProps> = ({ user, onClose, onSave, isAdmin = false, roles = [] }) => {
    ...
    const [visualViewport, setVisualViewport] = useState(() => ({ height: window.visualViewport?.height || window.innerHeight, top: window.visualViewport?.offsetTop || 0 }));

    useEffect(() => {
        const viewport = window.visualViewport;
        const updateViewport = () => setVisualViewport({ height: viewport?.height || window.innerHeight, top: viewport?.offsetTop || 0 });
        updateViewport();
            ...
            <div className="min-h-0 flex-1 space-y-5 overflow-y-auto overscroll-contain px-4 py-5 touch-pan-y [-webkit-overflow-scrolling:touch] sm:px-5">
                ...
                {!isAdmin && <>
                    <div className="rounded-2xl border border-white/10 bg-white/[.045] p-4">
                        ...
                        <p className="mt-1 text-[9px] text-slate-500">{so ? 'Cover-ku si otomaatig ah ayuu sawirkaaga uga samaysmaa. Dooro cidda furi karta card-kaaga.' : 'Your cover is created automatically from your photo. Choose who can open your card.'}</p>
                        ...
                        <p className="mt-3 text-[8px] font-semibold text-purple-300">{so ? 'Hoos ka taabo “Kaydi Isbeddelka” si dejintan server-ka loogu keydiyo.' : 'Tap “Save Changes” below to store these settings on the server.'}</p>
                    </div>
