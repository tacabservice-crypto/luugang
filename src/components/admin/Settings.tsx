      ...
      <div className="mt-6">
        {settingsView === 'security' && (
          <div className="max-w-3xl space-y-4">
            ...
            <div className="border-t border-gray-200 pt-5"><h3 className="text-xl font-bold">Phone Login</h3><p className="mt-1 text-sm text-gray-500">Control phone-and-password registration protected by an anti-spam security check. No SMS is sent.</p></div>
            <div className={`rounded-xl border p-5 ${phoneAuthEnabled ? 'border-emerald-200 bg-emerald-50' : 'border-amber-300 bg-amber-50'}`}>
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div><p className={`font-black ${phoneAuthEnabled ? 'text-emerald-800' : 'text-amber-900'}`}>Phone login is currently {phoneAuthEnabled ? 'ENABLED' : 'DISABLED'}</p><p className="mt-1 text-sm text-gray-600">{phoneAuthEnabled ? 'Users can register and sign in with phone, password and the security check.' : 'The Phone option is hidden and phone sign-in is rejected.'}</p></div>
                <button onClick={async () => { const next = !phoneAuthEnabled; if (!window.confirm(next ? 'Enable Phone login?' : 'Disable Phone login for all users?')) return; try { await onSavePhoneAuthSettings(next); setPhoneAuthEnabled(next); showNotification('success', `Phone login ${next ? 'enabled' : 'disabled'}.`); } catch (error: any) { showNotification('error', userErrorMessage(error, 'Phone login setting could not be saved.')); } }} className={`shrink-0 rounded-lg px-5 py-3 font-black text-white ${phoneAuthEnabled ? 'bg-red-600 hover:bg-red-700' : 'bg-emerald-600 hover:bg-emerald-700'}`}>{phoneAuthEnabled ? 'Disable Phone Login' : 'Enable Phone Login'}</button>
              </div>
            </div>
            <div className="rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm text-blue-800"><strong>Security note:</strong> Keep Turnstile keys configured on the server so bots cannot automate phone registrations.</div>
          </div>
        )}
        {settingsView === 'ads' && (
