...
var DEFAULT_REVENUE_SETTINGS = {
  gameRakeRate: RAKE_PERCENTAGE,
  spectatorBetCommissionRate: 0.1,
  botWinCharge: 0.02,
  botPlayerWinReward: 0.01,
  noPlayWithdrawalFeeRate: 0.1,
  minimumWithdrawalFee: 0.1,
  normalWithdrawalFeeRate: 0,
  tournamentCommissionRate: 0.1,
...
function normalizeAppAvatar(value) {
  ...
  if (!avatar) return "\u{1F3AE}";
  if (!/^https?:\/\//i.test(avatar)) return avatar;
  try {
    const hostname = new URL(avatar).hostname.toLowerCase();
    return hostname === "googleusercontent.com" || hostname.endsWith(".googleusercontent.com") ? avatar : "\u{1F3AE}";
  } catch {
    return "\u{1F3AE}";
  }
}
function verifiedGoogleAvatar(req) {
...
function shouldUseGoogleAvatar(currentAvatar) {
  const avatar = typeof currentAvatar === "string" ? currentAvatar.trim() : "";
  if (!avatar || avatar === "\u{1F3AE}") return true;
  try {
    const hostname = new URL(avatar).hostname.toLowerCase();
...
function getSpectatorMarketMembers(room, targetPlayerId) {
...
function getSpectatorMarketTarget(room, targetPlayerId) {
...
function getWithdrawalQuote(userId, amount) {
  ...
  const fee = feeRate <= 0 ? 0 : Math.min(amount, playedPaidGame ? Number((amount * feeRate).toFixed(2)) : Math.max(store.revenueSettings.minimumWithdrawalFee, Number((amount * feeRate).toFixed(2))));
...
function moveTokenLogic(room, tokenId, diceValue) {
  ...
  if (hasWon) {
    ...
    if (isBotEconomyRoom(room)) {
      ...
      addLog(room, botWinnerIds.some((id) => !isBotPlayer(id)) ? `\u{1F3C6} Bot game won: each winning player receives $${store.revenueSettings.botPlayerWinReward.toFixed(2)}.` : `\u{1F916} Bot won: each human player is charged $${store.revenueSettings.botWinCharge.toFixed(2)}.`);
      return;
    }
    if (room.gameMode === "team") {
  ...
  candidateUsers.forEach((u) => {
    ...
    if (status === "seeking" || status === "online") {
      onlineList.push({
        ...
        createdAt: u.createdAt || Number(/^user_(\d+)/.exec(u.id)?.[1]) || void 0,
...
app.post("/api/users/:userId/message", verifyFirebaseToken, async (req, res) => {
  ...
  const message = { id: `dm_${now2}_${import_crypto.default.randomBytes(4).toString("hex")}`, senderId: sender.id, receiverId: receiver.id, senderName: sender.username, senderAvatar: sender.avatar, text, createdAt: now2 };
...
app.get("/api/rooms/active", (req, res) => {
  ...
  const activeGames = Object.values(store.rooms).filter((r) => {
    ...
    const myBet = currentUserId ? store.spectatorBets.find((bet) => bet.roomId === r.id && bet.userId === currentUserId) : void 0;
...
app.get("/api/rooms/:roomId/spectator-bet", (req, res) => {
  ...
  res.json({ bet, markets, commissionRate: store.revenueSettings.spectatorBetCommissionRate, minStake: 0.1, maxStake: 10 });
});
app.post("/api/rooms/:roomId/spectator-bet", async (req, res) => {
  ...
  const marketBets = store.spectatorBets.filter((bet2) => bet2.roomId === roomId && marketMemberIds.includes(bet2.targetPlayerId) && bet2.status === "open");
...
app.post("/api/rooms/challenge/invite", async (req, res) => {
  ...
  newRoom.challengeTargetUserId = receiverId;
  newRoom.challengeAttempt = 1;
  newRoom.challengeStatus = "pending";
  store.rooms[roomId] = newRoom;
  for (const qKey of Object.keys(store.matchmakingQueues)) {
...
app.post("/api/rooms/challenge/decline", async (req, res) => {
  ...
  if (room) {
    const hostId = room.players.find((p) => p.isHost)?.userId;
    const attempt = Math.max(1, Number(room.challengeAttempt || 1));
    ...
    if (hostId) {
      const payload = {
        ...
        room: isFinalAttempt ? void 0 : {
          ...room,
          challengeStatus: "declined",
          challengeReason: reason || "declined",
          invitedUserId: void 0
        }
      };
    ...
    if (isFinalAttempt) {
      ...
      room.challengeStatus = "declined";
      room.challengeReason = reason || "declined";
      delete room.invitedUserId;
      touchRoom(room);
      store.rooms[roomId] = room;
      await saveStoreAndWait();
...
app.post("/api/rooms/challenge/retry", async (req, res) => {
  ...
  if (room.status !== "waiting" || room.challengeStatus !== "declined") {
  ...
  const receiverId = String(room.challengeTargetUserId || "");
  let receiver = store.users[receiverId];
  if (!receiver && isMySqlConfigured()) receiver = await loadMySqlRuntimeUser(receiverId);
  if (!receiver) return res.status(409).json({ error: "Ciyaaryahankan hadda online ma aha." });
  room.challengeAttempt = attempt + 1;
  room.challengeStatus = "pending";
  room.invitedUserId = receiverId;
  delete room.challengeReason;
  touchRoom(room);
  store.rooms[room.id] = room;
  await saveStoreAndWait();
  await persistLiveRoom(room);
...
app.post("/api/admin/revenue-settings", hasPermission("all"), async (req, res) => {
  const rateKeys = [
    "gameRakeRate",
    "spectatorBetCommissionRate",
    "noPlayWithdrawalFeeRate",
    "normalWithdrawalFeeRate",
    "tournamentCommissionRate",
    "manualForfeitPenaltyRate",
    "inactivityPenaltyRate",
    "depositFeeRate",
    "privateMatchFeeRate"
  ];
  const amountKeys = ["botWinCharge", "botPlayerWinReward", "minimumWithdrawalFee"];
