...
app.post('/api/users/:userId/avatar', express.raw({ type: ['image/jpeg', 'image/png', 'image/webp'], limit: '2mb' }), async (req, res) => {
  ...
  await fs.promises.writeFile(path.join(avatarDirectory, filename), req.body, { flag: 'wx' });
  ...
  if (/^\/uploads\/avatars\/[a-f0-9]{20}-\d+\.(?:jpg|png|webp)$/i.test(previousAvatar)) {
    const previousPath = path.resolve(process.cwd(), 'public', previousAvatar.replace(/^\//, ''));
    const avatarRoot = path.resolve(avatarDirectory);
    if (previousPath.startsWith(`${avatarRoot}${path.sep}`)) void fs.promises.unlink(previousPath).catch(() => undefined);
  }
...
const parseGameStake = (value: unknown): number | null => {
